...
public interface KeyDao {
   ...
   *
   * @param id         to use.
   * @param createDate to use.
   * @param key        to use.
   * @param nonce      to use.
   */
  @SqlUpdate("insert into KEYS (id, createDate, key, nonce) values (:id, :createDate, :key, :nonce)")
  void insert(@Bind("id") String id, @Bind("createDate") Date createDate, @Bind("key") String key, @Bind("nonce") String nonce);
  ...
  Key findKeyById(@Bind("id") int id);
