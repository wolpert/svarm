...
public class NodeServiceManager implements ServiceManager {
  ...
  public void startup(EnvironmentConfiguration configuration) {
    SUPPORT = new DropwizardTestSupport<>(
        Node.class,
        ResourceHelpers.resourceFilePath("node-config.yaml")
    ...
    configuration.setNodeConnectionUrl("http://localhost:" + SUPPORT.getLocalPort() + "/");
  }
